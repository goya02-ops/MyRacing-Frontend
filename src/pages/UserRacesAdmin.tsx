...
import { Card, Badge, Divider } from '../components/tremor/TremorComponents';

// Tus nuevos componentes importados de 'HEAD'
...
export default function UserRaces() {
  ...
  // Lógica de carga de carreras (de 'HEAD')
  useEffect(() => {
    if (!selectedUserId) {
      setRaceUsers([]);
      return;
  ...
  // JSX fusionado, priorizando la estructura de 'HEAD'
  return (
    <Card className="text-gray-200">
      <h2 className="text-xl font-semibold mb-6">Carreras por Usuario</h2>

      <div className="flex flex-col md:flex-row gap-6">
        {/* Panel de lista de usuarios (de 'HEAD') */}
        <UserListPanel
          users={users}
        ...
        <div className="flex-1 min-w-0">
          ...
          {selectedUser && (
            <>
              <div className="flex flex-col md:flex-row justify-between md:items-center gap-2 mb-4">
                ...
                {!loadingRaces && (
                  <Badge variant="neutral" className="text-base">
                    Carreras visibles: {filteredRaceUsers.length} /{' '}
                    {raceUsers.length}
                  </Badge>
                )}
              </div>
              ...
              {/* Lista de carreras filtrada (de 'HEAD') */}
              {!loadingRaces && filteredRaceUsers.length > 0 && (
