...
export const AuthFormPanel: React.FC<AuthFormPanelProps> = ({ type, isActive, logic }) => {
  const isLogin = type === 'login';
  const showSuccess = !isLogin && !!logic.successMessage;

  // Lógica de animación para el panel
  const animationClasses = `${formPanelBaseClasses} left-0 z-${isLogin ? 20 : 10} ${
    isActive
  ...
  return (
    ...
    >
      <h1 className="text-3xl font-bold text-gray-100">{isLogin ? 'Iniciar Sesión' : 'Crear Cuenta'}</h1>
      <AuthSocialIcons />
