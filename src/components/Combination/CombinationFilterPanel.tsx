...
import { Label } from '../tremor/TremorComponents';
...
const CombinationFilterPanel: React.FC<CombinationFilterPanelProps> = ({
    ...
    filterSimulatorId, setFilterSimulatorId,
    filterCategoryId, setFilterCategoryId,
    filterCircuitId, setFilterCircuitId,
    filterUserType, setFilterUserType,
    filterDateFrom, setFilterDateFrom,
    filterDateTo, setFilterDateTo,
}) => {
    const inputClasses = "w-full p-2 rounded-md bg-gray-800 border border-gray-700 text-gray-200 placeholder-gray-500 focus:ring-orange-500 focus:border-orange-500 pr-8";
    ...
    return (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-6 p-4 bg-gray-900/50 rounded-lg border border-gray-800">
            ...
            <div>
                <Label htmlFor="filterSim" className="text-white">Simulador</Label>
                <select
                    id="filterSim"
                    value={filterSimulatorId === 'ALL' ? 'ALL' : filterSimulatorId.toString()}
                    onChange={(e) => handleSelectChange(setFilterSimulatorId, e.target.value)}
            ...
            <div>
                <Label htmlFor="filterCat" className="text-white">Categoría</Label>
                <select
                    id="filterCat"
                    value={filterCategoryId === 'ALL' ? 'ALL' : filterCategoryId.toString()}
                    onChange={(e) => handleSelectChange(setFilterCategoryId, e.target.value)}
                    className={inputClasses}
                >
            ...
            <div>
                <Label htmlFor="filterCirc" className="text-white">Circuito</Label>
                <select
                    id="filterCirc"
                    value={filterCircuitId === 'ALL' ? 'ALL' : filterCircuitId.toString()}
                    onChange={(e) => handleSelectChange(setFilterCircuitId, e.target.value)}
                    className={inputClasses}
                >
                    <option value="ALL">Todos</option>
                    {circuits.map(circ => (
            ...
            <div>
                <Label htmlFor="filterUser" className="text-white">Tipo de Usuario</Label>
                <select
                    id="filterUser"
            ...
            <div>
                <Label htmlFor="filterDateFrom" className="text-white">Desde (vigencia)</Label>
            ...
            <div>
                <Label htmlFor="filterDateTo" className="text-white">Hasta (vigencia)</Label>
                <div className="relative">
                    <input
                        id="filterDateTo"
                        type="date"
