...
import { Simulator, Category, Circuit, CategoryVersion, CircuitVersion } from '../types/entities';
...
export function useVersionManagerLogic(
        ...
        const fetchVersions = async () => {
            try {
                if (activeManager.type === 'category') {
                    ...
                    setCategoryVersions(
                        allVersions.filter((v: any) => getRelationId(v, 'simulator') === activeManager.simulator?.id) as CategoryVersion[]
                    );
                } else if (activeManager.type === 'circuit') {
                    const allVersions = await fetchEntities(CircuitVersion as any);
                    setCircuitVersions(
                        allVersions.filter((v: any) => getRelationId(v, 'simulator') === activeManager.simulator?.id) as CircuitVersion[]
                    );
                }
            } catch (error) {
    ...
    const handleEditVersion = useCallback((version: CategoryVersion | CircuitVersion) => {
    ...
    const isDuplicate = useCallback((version: any, entityType: 'category' | 'circuit') => {
        return currentVersions.some(
            (item: any) =>
                getRelationId(item, entityType) === getRelationId(version, entityType) &&
                getRelationId(item, 'simulator') === getRelationId(version, 'simulator') &&
                item.id !== version.id
        );
    }, [currentVersions]);

    const onSaveCategoryVersion = useCallback((v: CategoryVersion) => {
        handleSaveEntity(
            CategoryVersion as any,
            v,
            setCategoryVersions as React.Dispatch<React.SetStateAction<CategoryVersion[]>>,
            handleCancelVersion,
            (version: CategoryVersion) => isDuplicate(version, 'category') // Función de chequeo
        );
    }, [handleSaveEntity, handleCancelVersion, isDuplicate]);

    const onSaveCircuitVersion = useCallback((v: CircuitVersion) => {
        handleSaveEntity(
            CircuitVersion as any,
            v,
            setCircuitVersions as React.Dispatch<React.SetStateAction<CircuitVersion[]>>,
            handleCancelVersion,
